...
class Music:
    ...
    async def purge(self, ctx, *, limit: str):
        ...
        if (limit.lower() != 'all'):
            try:
                int_limit = int(limit)
                if int_limit > PURGE_LIMIT:
                    int_limit = PURGE_LIMIT
            except ValueError:
                await self.bot.say('From the last how many messages should bot messaged be purged (Max={}).'.format(PURGE_LIMIT))
        await self.bot.delete_message(ctx.message)
        await self.bot.purge_from(channel, limit=int_limit, check=lambda x: x.author == self.bot.user)
